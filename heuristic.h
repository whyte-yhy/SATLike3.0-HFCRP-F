...
void Satlike::init(vector<int> &init_solution)
{
    ...
    for (int c = 0; c < num_clauses; c++)
    {
        ...
        else
        {
            ...
            {
                clause_weight[c] = basis_clause_weight[c] + org_clause_weight[c];
                //basis_clause_weight[c] -= basis_clause_weight[c]>=basis_decrease_unit ? basis_decrease_unit : 0;
                //basis_clause_weight[c] = 0;
                //clause_weight[c] = basis_clause_weight[c];
...
void Satlike::local_search_with_decimation(char *inputfile)
{
    ...
    for (tries = 1; tries < max_tries; ++tries)
    {
        if (best_soln_feasible != 1)
        {
            deci.init(local_opt_soln, best_soln, unit_clause, unit_clause_count, clause_lit_count, best_soln_feasible);
            deci.unit_prosess();
            init(deci.fix);
        }
        else {
...
//                    }
                ...
                if (opt_unsat_weight > soft_unsat_weight)
                {
                    ...
                    cout << "o " << soft_unsat_weight << " tries:" << tries <<endl;
                }
            ...
            if (step % 1000 == 0 || opt_unsat_weight == 0) {
                double elapse_time = get_runtime();
                if (elapse_time >= 300 || opt_unsat_weight == 0) {
...
//                    cout << endl;
                ...
                else if (elapse_time >= cutoff_time && print60 == 0) {
                    print60 = 1;
                    cout << "time limit:" << cutoff_time << "s" << endl;
                    simple_print();
                }
            }
